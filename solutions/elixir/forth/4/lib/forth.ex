defmodule Forth do
  ...
  @spec new() :: evaluator
  def new() do
    %Evaluator{:stack => [], :program => [], :dictionary => %{}}
  end
  ...
  def eval(ev, s) do
    %Evaluator{:stack => stack, :program => _program, :dictionary => dict} = ev
    program = tokenize(s)
    {result_stack, result_program, result_dict} = eval_rec(stack, program, dict)
    %Evaluator{:stack => result_stack, :program => result_program, :dictionary => result_dict}
  end
  ...
  def format_stack(ev) do
    %Evaluator{:stack => stack, :dictionary => _dict} = ev
